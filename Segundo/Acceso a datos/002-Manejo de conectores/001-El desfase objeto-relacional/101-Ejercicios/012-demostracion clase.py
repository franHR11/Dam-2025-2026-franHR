...
class JsonMySQLBridge:
    """
    Transforma recursivamente entre JSON y MySQL.
    Regla principal: cada clave del objeto raíz del JSON representa una TABLA DE NIVEL SUPERIOR.
    Las subestructuras (dict/lista) se convierten en tablas hijas con FK hacia el padre.
    Las listas de valores escalares se guardan como (parent_id, idx, valor).
    """

    # ===================== CONSTRUCCIÓN =====================
    def __init__(self, host, user, password, database, json_path_default="./datos.json"):
        self.DB = dict(database=database, host=host, user=user, password=password)
    ...
    def _ident(name: str) -> str:
        """Limpia identificadores para MySQL (tabla/columna)."""
